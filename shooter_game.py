...
class Player(GameSprite):
    ...
    def fire(self):
        bullet = Bullet('yhm.png', self.rect.centerx, self.rect.top, 15, 20, 15)
        bullets.add(bullet) 
...
player = Player('rocket.png', 5, win_height-100, 60, 80, 5)
enemies = sprite.Group()
for i in range(1,6):
    enemy = Enemy('r.png', randint(-10, win_width-80), -50, 80, 50, randint(1,5))
    enemies.add(enemy)
asteroids = sprite.Group()
for i in range(1,3):
    asteroid = Enemy('asteroid.png', randint(80, win_width-80), -50, 60, 70, randint(1,5))
    asteroids.add(asteroid)
bullets = sprite.Group()
fire_sound = mixer.Sound('yy.mp3')
...
while run:
    ...
    if not finish:        
        ...
        for c in collides:
            score = score + 1
            enemy = Enemy('r.png', randint(80, win_width-80), -50, 60, 70, randint(1,5))
            enemies.add(enemy)
        ...
        if sprite.spritecollide(player, enemies, False) or missed > 5:
            finish = True
            font2 = font.Font(None, 100)
            lose = font2.render('YOU LOSE!', True, (180,0,0))
            window.blit(lose, (180, 200))  
